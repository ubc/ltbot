...
STORAGE = os.environ.get('STORAGE', 'Shelf')  # defaults to filestorage (python shelf).

# extra search path to find custom storage plugins
BOT_EXTRA_STORAGE_PLUGINS_DIR = os.environ.get('BOT_EXTRA_STORAGE_PLUGINS_DIR', None)
...
BOT_PLUGIN_INDEXES = os.environ.get('BOT_PLUGIN_INDEXES', 'https://repos.errbot.io/repos.json')
#
...
# The environment variable should be set to something like PLUGINS_CALLBACK_ORDER=A,B,C
PLUGINS_CALLBACK_ORDER = os.environ.get('PLUGINS_CALLBACK_ORDER').split(',') \
    if 'PLUGINS_CALLBACK_ORDER' in os.environ else (None, )
...
AUTOINSTALL_DEPS = os.environ.get('AUTOINSTALL_DEPS') == 'True' \
    if 'AUTOINSTALL_DEPS' in os.environ else True
...
# single command.
BOT_ASYNC = os.environ.get('BOT_ASYNC') == 'True' \
    if 'BOT_ASYNC' in os.environ else True

# Size of the thread pool for the asynchronous mode.
BOT_ASYNC_POOLSIZE = int(os.environ.get('BOT_ASYNC_POOLSIZE', 10))
...
for i in ['BOT_USERNAME', 'BOT_PASSWORD', 'BOT_TOKEN', 'BOT_ENDPOINT', 'BOT_NICKNAME', 'BOT_PORT', 'BOT_SSL']:
    if i in os.environ:
        BOT_IDENTITY[i[4:].lower()] = os.environ[i]
...
BOT_ADMINS = tuple(
    os.environ.get('BOT_ADMINS', '@admin').split(','),
)

# Set of admins that wish to receive administrative bot notifications.
BOT_ADMINS_NOTIFICATIONS = tuple(
    os.environ.get('BOT_ADMINS_NOTIFICATIONS', '').split(','),
)
...
CHATROOM_PRESENCE = tuple(
    os.environ.get('CHATROOM_PRESENCE',
                   '#errbotroom').split(','),
) if 'CHATROOM_PRESENCE' in os.environ else tuple()
...
# the prefix above.
if 'BOT_ALT_PREFIXES' in os.environ:
...
XMPP_KEEPALIVE_INTERVAL = int(os.environ.get('XMPP_KEEPALIVE_INTERVAL', 300))

# Modify default settings for IPv6 usage. This key affect both
# XMPP and HipChat backend.
XMPP_USE_IPV6 = bool(os.environ.get('XMPP_USE_IPV6', False))
...
XMPP_XHTML_IM = bool(os.environ.get('XMPP_XHTML_IM', False))
...
IRC_ACL_PATTERN = os.environ.get('IRC_ACL_PATTERN', "{nick}!{user}@{host}")

# Allow messages sent in a chatroom to be directed at requester.
GROUPCHAT_NICK_PREFIXED = bool(os.environ.get('GROUPCHAT_NICK_PREFIXED', False))

# Disable table borders, making output more compact (supported only on IRC, Slack and Telegram currently).
COMPACT_OUTPUT = bool(os.environ.get('COMPACT_OUTPUT', False))

# Disables the logging output in Text mode and only outputs Ansi.
TEXT_DEMO_MODE = bool(os.environ.get('TEXT_DEMO_MODE', False))

# Prevent ErrBot from saying anything if the command is unrecognized.
SUPPRESS_CMD_NOT_FOUND = bool(os.environ.get('SUPPRESS_CMD_NOT_FOUND', False))
